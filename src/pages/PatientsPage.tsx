...
export default function PatientsPage() {
  ...
  const loadPatients = async () => {
    if (!hospitalId) return; // Add guard clause
    setLoading(true)
    try {
      const result = await patientService.getPatients(hospitalId)
      if (result.success) {
  ...
  const deletePatient = async () => {
    ...
    try {
      // In a real application, you'd call a patientService.deletePatient method
      ...
      setIsDeleteModalOpen(false)
      // No need to call loadPatients as we've updated local state
