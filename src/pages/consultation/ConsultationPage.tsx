...
import { appointmentService, patientService } from '@/services' // Corrected: import from the services barrel file
// ------------------------------
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
...
export default function ConsultationPage() {
  ...
  const loadAppointmentData = async () => {
    ...
    setLoading(true)
    try {
      // In a real app, appointmentService would have a getById method
      // We will simulate this for now
      // const appointmentResult = await appointmentService.getAppointmentById(appointmentId, hospitalId)
      const appointmentResult = { success: true, data: { id: appointmentId, patient_id: 'mock-patient-id', diagnosis: 'Fever', notes: 'Rest for 2 days', vital_signs: {}, prescription: { medications: [] } } };
      ...
      if (appointmentResult.data.patient_id) {
        // const patientResult = await patientService.getPatientById(appointmentResult.data.patient_id, hospitalId)
        const patientResult = { success: true, data: { id: 'mock-patient-id', personal_info: { first_name: 'John', last_name: 'Doe', photo_url: '', gender: 'Male', age: 30 }, uhid: 'UHID123', medical_info: { chronic_conditions: ['Hypertension'], allergies: ['Pollen'] } } };
  ...
  const saveConsultation = async () => {
    ...
    try {
      ...
      //   hospitalId
      // )
      console.log("Saving consultation data:", updatedAppointment);
