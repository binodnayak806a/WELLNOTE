...
import { Calendar, Plus, Search, Filter, Clock, CheckCircle, XCircle, MoreVertical, Edit, Trash2, Loader2 } from 'lucide-react'
import { toast } from 'sonner'
import { useAuth } from '@/hooks/useAuth'
...
type AppointmentWithDetails = Appointment & {
  patient: Pick<Patient, 'id' | 'personal_info'>;
  doctor: Pick<Doctor, 'id' | 'personal_info'>;
}
...
export default function AppointmentsPage() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      const [doctorsResult, patientsResult] = await Promise.all([
        doctorService.getDoctors(hospitalId),
  ...
  const loadAppointments = async () => {
    ...
    setLoading(true)
    try {
      const result = await appointmentService.getAppointmentsByDate(selectedDate, hospitalId)
      if (result.success) {
        const appointmentsData = result.data || []
        const detailedAppointments = appointmentsData.map(appt => ({
          ...appt,
          patient: patients.find(p => p.id === appt.patient_id)?.personal_info || { first_name: 'Unknown', last_name: 'Patient' },
          doctor: doctors.find(d => d.id === appt.doctor_id)?.personal_info || { first_name: 'Unknown', last_name: 'Doctor' }
        }));
        setAppointments(detailedAppointments as AppointmentWithDetails[])
      } else {
  ...
  const handleStatusChange = async (appointmentId: string, newStatus: 'confirmed' | 'cancelled' | 'completed') => {
    // In a real app, you would call an update service
    try {
  ...
  const filteredAppointments = appointments.filter(appt => {
    ...
    const searchMatch = !searchQuery || patientName.includes(searchQuery.toLowerCase())
    const doctorMatch = selectedDoctor === 'all' || appt.doctor_id === selectedDoctor
    const statusMatch = selectedStatus === 'all' || appt.status === selectedStatus
    return searchMatch && doctorMatch && statusMatch
  }).sort((a, b) => new Date(`1970-01-01T${a.appointment_time}`).getTime() - new Date(`1970-01-01T${b.appointment_time}`).getTime())
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setIsModalOpen(true)} className="bg-medical-600 hover:bg-medical-700">
          <Plus className="w-4 h-4 mr-2" />
          New Appointment
        </Button>
      </div>
                ...
                <SelectContent>
                  ...
                  {doctors.map(doc => (
                    <SelectItem key={doc.id} value={doc.id}>
                      Dr. {doc.personal_info.first_name} {doc.personal_info.last_name}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    ...
                    <th className="px-4 py-3 text-left font-medium text-gray-900">Doctor</th>
                    ...
                    <tr key={appt.id} className="border-b hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium text-gray-800">
                        <div className="flex items-center">
                          ...
                          {new Date(`1970-01-01T${appt.appointment_time}`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <p className="font-medium">{appt.patient.first_name} {appt.patient.last_name}</p>
                        <p className="text-sm text-gray-500">{appt.appointment_type}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <Select 
                          onValueChange={(newStatus) => handleStatusChange(appt.id, newStatus as any)}
